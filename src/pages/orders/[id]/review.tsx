...
export default function ReviewOrderPage() {
        ...
        const fetchData = async () => {
            try {
                ...
                if (res.data && res.data.products) {
                    setProducts(res.data.products); // Lưu dữ liệu sản phẩm
                }

                // Lấy đánh giá cho mỗi sản phẩm
                for (let prod of res.data.products) {
                    const reviewsRes = await axios.get(`http://localhost:8000/api/reviews/${prod.product_id}`);
                    setReviewData((prev) => ({
                        ...prev,
                        [prod.product_id]: reviewsRes.data,
                    }));
                }
            } catch (err) {
    ...
    const handleReview = async (product_id: number) => {
        ...
        try {
            ...
            // Hiển thị thông báo thành công mà không điều hướng
            toast.success("Đánh giá thành công!");

            // Cập nhật trạng thái sản phẩm để hiển thị rằng sản phẩm đã được đánh giá
            setProducts((prev) =>
                prev.map((p) =>
                    p.product_id === product_id ? { ...p, is_reviewed: true, rating, comment } : p
                )
            );
        } catch (err: any) {
